...
const createICard = (data) => {
    ...
    async function drawIdcard() {
        ...
        ctx.ellipse(415, 440, 168, 182, Math.PI, 0, 2 * Math.PI);
        // ellipse(x, y, radiusX, radiusY, rotation, startAngle, endAngle, counterclockwise)
        ctx.fill();
        const stuImgW = 336;
        const stuImgH = stuImgW / stuImgRatio;
        console.log(stuImgW, stuImgH);
        const stuImg = new Image();
        const stuImgPromise = new Promise((resolve, reject) => {
            stuImg.onload = function () {
                ctx.drawImage(stuImg, 247, 240, stuImgW, stuImgH);
                // stuImgLoaded = true;
                resolve(true);
            };
        });
